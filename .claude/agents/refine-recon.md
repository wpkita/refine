...
- **Mechanical lenses**: run only the deterministic checks you are asked to run, and return raw findings with file:line references. This includes the machine-checkable rungs of `.claude/skills/refine/maturity.md` — the source of truth for what Refine prioritizes — (file existence, CI config contents, audit output, history scans): report each as pass/fail with the rung number.
